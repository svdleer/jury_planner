...
<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    ...
    <div class="bg-white shadow-sm ring-1 ring-gray-900/5 rounded-lg mb-6">
        <div class="px-4 py-5 sm:p-6">
            ...
            <p class="text-sm text-gray-600 mb-4"><?php echo t('set_capacity_description'); ?></p>
                ...
                <?php foreach ($teams as $team): ?>
                    <div class="border rounded-lg p-4">
                        <form method="POST" class="space-y-3">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700"><?php echo t('capacity_factor'); ?></label>
    ...
    <div class="bg-white shadow-sm ring-1 ring-gray-900/5 rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4"><?php echo t('current_exclusion_constraints'); ?></h3>
            
            <?php if (isset($exclusionError)): ?>
                <div class="text-red-600 mb-4"><?php echo t('error_loading_exclusions'); ?>: <?php echo htmlspecialchars($exclusionError); ?></div>
            <?php endif; ?>
            
            <?php if (empty($exclusions)): ?>
                <p class="text-gray-500"><?php echo t('no_exclusion_constraints_defined'); ?></p>
            <?php else: ?>
                <div class="space-y-2">
                    <?php foreach ($exclusions as $exclusion): ?>
                        <div class="border rounded p-3 flex justify-between items-center">
                            <p><strong><?php echo htmlspecialchars($exclusion['name']); ?></strong> <?php echo t('is_excluded_from_jury_duty'); ?></p>
                            <form method="POST" class="inline">
                                ...
                                <button type="submit" 
                                        ...
                                        onclick="return confirm('<?php echo t('remove_this_exclusion'); ?>')">
                                    <?php echo t('remove'); ?>
                                </button>
                            </form>
                        </div>
    ...
    <div class="bg-white shadow-sm ring-1 ring-gray-900/5 rounded-lg mb-6">
        <div class="px-4 py-5 sm:p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4"><?php echo t('custom_constraints'); ?></h3>
            ...
            <div class="border-b border-gray-200 pb-6 mb-6">
                ...
                <form method="POST" class="space-y-4" x-data="{ constraintType: '', showFields: {} }">
                    ...
                    <div>
                        <label for="constraint_type" class="block text-sm font-medium text-gray-700 mb-1"><?php echo t('constraint_type'); ?></label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label for="source_team" class="block text-sm font-medium text-gray-700 mb-1"><?php echo t('source_team'); ?></label>
                        ...
                        <div x-show="constraintType === 'team_team_conflict'">
                            <label for="target_team" class="block text-sm font-medium text-gray-700 mb-1"><?php echo t('target_team'); ?></label>
                            <select name="target_team"
                                    class="block w-full rounded-md border-gray-300 shadow-sm focus:border-water-blue-500 focus:ring-water-blue-500 sm:text-sm">
                                <option value="">-- <?php echo t('select_target_team'); ?> --</option>
                                <?php foreach ($teams as $team): ?>
                                    <option value="<?php echo htmlspecialchars($team['name']); ?>">
                                        <?php echo htmlspecialchars($team['name']); ?>
                                    </option>
                        ...
                        <div x-show="constraintType === 'date_restriction'">
                            <label for="constraint_date" class="block text-sm font-medium text-gray-700 mb-1"><?php echo t('date'); ?></label>
                        ...
                        <div x-show="constraintType === 'capacity_override' || constraintType === 'assignment_limit'">
                            <label for="constraint_value" class="block text-sm font-medium text-gray-700 mb-1">
                                <?php echo t('value'); ?> <span x-show="constraintType === 'capacity_override'">(0.1 - 5.0)</span>
                            </label>
                    ...
                    <div>
                        <label for="reason" class="block text-sm font-medium text-gray-700 mb-1"><?php echo t('reason'); ?></label>
                        <textarea name="reason" rows="2"
                                  class="block w-full rounded-md border-gray-300 shadow-sm focus:border-water-blue-500 focus:ring-water-blue-500 sm:text-sm"
                                  placeholder="<?php echo t('optional_explanation_constraint'); ?>"></textarea>
                    </div>
            ...
            <?php if (empty($customConstraints)): ?>
                <p class="text-gray-500"><?php echo t('no_custom_constraints_defined'); ?></p>
            <?php else: ?>
                <div class="space-y-3">
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center gap-2 mb-2">
                                        ...
                                        <?php if (!$constraint['is_active']): ?>
                                            <span class="text-xs text-gray-500">(<?php echo t('disabled'); ?>)</span>
                                        <?php endif; ?>
                                    </div>
                                    ...
                                    <p class="text-sm text-gray-900">
                                        ...
                                        <?php endif; ?>
                                        <?php if ($constraint['constraint_date'] && $constraint['constraint_date'] !== '0000-00-00' && $constraint['constraint_date'] !== '1969-12-31'): ?>
                                            <?php echo t('on'); ?> <?php echo date('M j, Y', strtotime($constraint['constraint_date'])); ?>
                                        <?php endif; ?>
                                        <?php if ($constraint['constraint_value'] && $constraint['constraint_value'] > 0): ?>
                                            (<?php echo t('value'); ?>: <?php echo number_format($constraint['constraint_value'], 2); ?>)
                                        <?php endif; ?>
                                    </p>
                                ...
                                <div class="flex gap-2 ml-4">
                                    <form method="POST" class="inline">
                                        ...
                                        <button type="submit" 
                                            ...
                                            <?php echo $constraint['is_active'] ? t('disable') : t('enable'); ?>
                                        </button>
                                    </form>
                                    ...
                                    <form method="POST" class="inline">
                                        ...
                                        <button type="submit" 
                                                ...
                                                onclick="return confirm('<?php echo t('delete_this_constraint'); ?>')">
                                            <?php echo t('delete'); ?>
                                        </button>
                                    </form>
                                </div>
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        <div class="flex">
            ...
            <div class="ml-3">
                <h3 class="text-sm font-medium text-blue-800"><?php echo t('custom_constraints_available'); ?></h3>
                <div class="mt-2 text-sm text-blue-700">
                    <p><?php echo t('run_setup_script_constraints'); ?>:</p>
